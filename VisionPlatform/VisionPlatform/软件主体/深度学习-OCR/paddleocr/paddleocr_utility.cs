...
namespace PaddleOCR
{
    ...
    {
        public OpenCvSharp.Point[] box = new OpenCvSharp.Point[4];
        public string text = "";
    ...
    public class PaddleOcrUtility
    {
        ...
        public static void print_result(List<OCRPredictResult> ocr_result)
        {
            for (int i = 0; i < ocr_result.Count; i++)
            {
                ...
                // det
                Point[] boxes = ocr_result[i].box;
                if (boxes.Length > 0)
                {
                    mes += " det boxes:[";
                    for (int n = 0; n < boxes.Length; n++)
                    {
                        mes += String.Format("[{0}, {1}]", boxes[n].Y, boxes[n].X);
                        if (n != boxes.Length - 1)
                        {
                            mes += ',';
                        }
        ...
        public static Mat visualize_bboxes(Mat srcimg, List<OCRPredictResult> ocr_result)
        {
            ...
            for (int n = 0; n < ocr_result.Count; n++)
            {
                ...
                rook_points[0] = ocr_result[n].box[0];
                rook_points[1] = ocr_result[n].box[1];
                rook_points[2] = ocr_result[n].box[2];
                rook_points[3] = ocr_result[n].box[3];
            ...
            for (int n = 0; n < ocr_result.Count; n++) 
            {
                int w = (int)Math.Ceiling((double)(ocr_result[n].box[1].X - ocr_result[n].box[0].X) / 3.0) + 1;
                int h = (int)Math.Ceiling((double)(ocr_result[n].box[2].Y - ocr_result[n].box[0].Y) / 3.0) + 1;
                int min = w < h ? w : h;
                Font font = new Font("Arial", min);
                float y = (float)ocr_result[n].box[0].X;
                if (y > min * 1.5) 
                {
                    y -= (int)(min*1.5);
                }
                // 设置文本位置（左上角）
               PointF point = new PointF(ocr_result[n].box[0].Y, y);
        ...
        public static Mat get_rotate_crop_image1(Mat srcimage, List<List<int>> box)
        {
        ...
        public static Mat get_rotate_crop_image(Mat srcimage, OpenCvSharp.Point[] box)
        {
            Mat dst_img = new Mat();
            ...
            {
                Mat image = srcimage.Clone();
                points = Utility.Clone<OpenCvSharp.Point[]>(box);

                List<int> x_collect = new List<int> { (int)box[0].X, (int)box[1].X, (int)box[2].X, (int)box[3].X };
                List<int> y_collect = new List<int> { (int)box[0].Y, (int)box[1].Y, (int)box[2].Y, (int)box[3].Y };
                int left = x_collect.Min();
                int right = x_collect.Max();
                int top = y_collect.Min();
                int bottom = y_collect.Max();
                ...
                int img_crop_height = (int)Math.Sqrt(Math.Pow(points[0].Y - points[1].Y, 2) +
                                              Math.Pow(points[0].X - points[1].X, 2));
                int img_crop_width = (int)Math.Sqrt(Math.Pow(points[0].Y - points[2].Y, 2) +
                                               Math.Pow(points[0].X - points[2].X, 2));
                //必须要(Row,Col)左上、左下、右下、右上
                Point2f[] pts_std = new Point2f[4];
                // 变换后坐标点
                pts_std[0] = new Point2f(0.0f, 0.0f);
                pts_std[1] = new Point2f(img_crop_width, 0.0f);
                ...
                // 变换前坐标点
                Point2f[] pointsf = new Point2f[4];
                pointsf[0] = new Point2f(points[0].X, points[0].Y);
                pointsf[3] = new Point2f(points[1].X, points[1].Y);
                pointsf[1] = new Point2f(points[2].X, points[2].Y);
                pointsf[2] = new Point2f(points[3].X, points[3].Y);
                // 获取变化矩阵
                Mat M = Cv2.GetPerspectiveTransform(pointsf, pts_std);
                Cv2.WarpPerspective(img_crop, dst_img, M, new Size(img_crop_width, img_crop_height),
                    InterpolationFlags.Linear, BorderTypes.Replicate);
        ...
        {
            ocr_result = ocr_result.OrderBy(t => t.box[0].X).ThenBy(t => t.box[0].Y).ToList();
            if (ocr_result.Count > 0)
            {
                for (int i = 0; i < ocr_result.Count - 1; i++)
                {
                    for (int j = i; j >= 0; j--)
                    {
                        if (Math.Abs(ocr_result[j + 1].box[0].X - ocr_result[j].box[0].X) < 10 &&
                            (ocr_result[j + 1].box[0].Y < ocr_result[j].box[0].Y))
                        {
