...
namespace PaddleOCR
{
    public static class PostProcessor
    {
        ...
        static List<List<int>> order_points_clockwise1(List<List<int>> pts)
        {
        ...
        static OpenCvSharp.Point[] order_points_clockwise(OpenCvSharp.Point[] pts)
        {
            List<List<int>> box = new List<List<int>>();
            List<int> point = new List<int>();
            foreach (OpenCvSharp.Point pt in pts)
            {
                point = new List<int>();
                point.Add(pt.X);
                point.Add(pt.Y);
            ...
            OpenCvSharp.Point[] rect = new OpenCvSharp.Point[4];
            if (leftmost[0][1] > leftmost[1][1])
            {
                rect[0] = new Point() { X = leftmost[1][0], Y = leftmost[1][1] };
                rect[1] = new Point() { X = leftmost[0][0], Y = leftmost[0][1] };
            }
            else
            {
                rect[0] = new Point() { X = leftmost[0][0], Y = leftmost[0][1] };
                rect[1] = new Point() { X = leftmost[1][0], Y = leftmost[1][1] };
            }

            if (rightmost[0][1] > rightmost[1][1])
            {
                rect[2] = new Point() { X = rightmost[1][0], Y = rightmost[1][1] };
                rect[3] = new Point() { X = rightmost[0][0], Y = rightmost[0][1] };
            }
            else
            {
                rect[2] = new Point() { X = rightmost[0][0], Y = rightmost[0][1] };
                rect[3] = new Point() { X = rightmost[1][0], Y = rightmost[1][1] };
            }
            return rect;
        }
        ...
        public static List<List<List<int>>> boxes_from_bitmap1(Mat pred, Mat bitmap, float box_thresh, float det_db_unclip_ratio, string det_db_score_mode)
        {
        ...
        public static List<OpenCvSharp.Point[]> boxes_from_bitmap(Mat pred, Mat bitmap, float box_thresh, float det_db_unclip_ratio, string det_db_score_mode)
        {
            const int min_size = 3;
            const int max_candidates = 1000;
            ...
            for (int _i = 0; _i < num_contours; _i++)
            {
                ...
                int dest_width = pred.Cols;
                int dest_height = pred.Rows;
                OpenCvSharp.Point[] intcliparray = new OpenCvSharp.Point[4];
                for (int num_pt = 0; num_pt < 4; num_pt++)
                {
                    OpenCvSharp.Point a = new OpenCvSharp.Point
                    {
                        X = (int)clampf((float)Math.Round(cliparray[num_pt][0] / (float)(width) * (float)(dest_width)), 0, (float)(dest_width)),
                        Y = (int)clampf((float)Math.Round(cliparray[num_pt][1] / (float)(height) * (float)(dest_height)), 0, (float)(dest_height))
                    };
        ...
        public static List<List<List<int>>> filter_tag_det_res1(List<List<List<int>>> boxes, float ratio_h, float ratio_w, Mat srcimg)
        {
            ...
            {
                boxes[n] = order_points_clockwise1(boxes[n]);
        ...
        public static List<OpenCvSharp.Point[]> filter_tag_det_res(List<OpenCvSharp.Point[]> boxes, float ratio_h, float ratio_w, Mat srcimg)
        {
            int oriimg_h = srcimg.Rows;
            int oriimg_w = srcimg.Cols;
            List<OpenCvSharp.Point[]> root_points = new List<OpenCvSharp.Point[]>();
            for (int n = 0; n < boxes.Count(); n++)
            {
                boxes[n] = order_points_clockwise(boxes[n]);
                for (int m = 0; m < boxes[0].Count(); m++)
                {
                    boxes[n][m].Y = (int)(boxes[n][m].Y / ratio_h);
                    boxes[n][m].X = (int)(boxes[n][m].X / ratio_w);

                    boxes[n][m].X = Math.Min(Math.Max(boxes[n][m].X, 0), oriimg_w - 1);
                    boxes[n][m].Y = Math.Min(Math.Max(boxes[n][m].Y, 0), oriimg_h - 1);
                }
            }
            ...
            {
                int rect_width, rect_height;
                rect_width = (int)(Math.Sqrt(Math.Pow(boxes[n][0].Y - boxes[n][1].Y, 2) +
                                      Math.Pow(boxes[n][0].X - boxes[n][1].X, 2)));
                rect_height = (int)(Math.Sqrt(Math.Pow(boxes[n][0].Y - boxes[n][3].Y, 2) +
                                       Math.Pow(boxes[n][0].X - boxes[n][3].X, 2)));
                if (rect_width <= 4 || rect_height <= 4)
                    continue;
                root_points.Add(boxes[n]);
            }
